<template>
  <v-container fluid class="fill-height" id="welcome-screen-root">
    ...
    <div style="position: absolute; top: 192px; left: 72px">
      <div
        v-if="authStore.displayName != null && authStore.servers.length > 0"
        class="welcome"
      >
      ...
      <v-dialog max-width="500" persistent no-click-animation>
        <template v-slot:activator="{ props: activatorProps }">
          <v-btn
            ...
            :disabled="
              authStore.displayName == null || authStore.servers.length == 0
            "
            >Create room</v-btn
          >
        </template>
        ...
        <template v-slot:default="{ isActive }">
          <v-card>
              ...
              >
                <template v-slot:prepend>
                  <v-icon
                    ...
                    v-if="server.publicAddress != creatingRoomFor"
                  ></v-icon>
                  <v-progress-circular
                    color="white"
                    indeterminate
            ...
            <v-alert
              density="compact"
              :text="creatingRoomError"
              v-if="creatingRoomError != null"
              title="Error"
              type="error"
            ></v-alert>
          ...
          <v-card>
            ...
            <v-card-text>
              ...
              <v-radio-group class="ml-4" v-model="subsPreference">
                ...
                <v-radio
                  label="Prefer full subtitles when audio is not in english"
                  value="full"
                />
                <v-radio
                  label="Always prefer full subtitles"
                  value="always-full"
                />
              </v-radio-group>
                  ...
                  <v-expansion-panel>
                    ...
                    <template v-slot:text>
                      <v-form
                        ...
                        @submit.prevent="addJellyfinServer"
                        class="ml-12"
                      >
                        <v-text-field
                          v-model="loginFormServerAddress"
                          autofocus
                          variant="underlined"
                          density="comfortable"
                          hide-details="auto"
                          placeholder="Server address (e.g. https://jellyfin.example.org)"
                          required
                        ></v-text-field>
                        <v-text-field
                          v-model="loginFormUsername"
                          variant="underlined"
                          density="comfortable"
                          hide-details="auto"
...
export default {
  ...
  methods: {
    ...
    async joinRoomVue(code: string) {
      if (this.authStore.displayName != null) {
        ...
        if (typeof result === "string") {
          ...
          if (server == undefined) {
            console.log(
              "Leaving because the required jellyfin server was not found"
            );
    ...
    getURLParameter(sParam: string): string | null {
  ...
  mounted() {
    let room = this.getURLParameter("room");
    if (room != null) {
