  ...
  <div class="sidebar sidebar-docked d-flex flex-column" v-if="pinned">
    ...
    <div>
      <v-expand-transition group>
        <div
          v-for="user in syncStore.session?.members"
          :key="user.index"
        ...
        >
          <div class="mr-1">
            <v-icon
              size="x-large"
              v-if="playbackStatus(user.index) == 'unknown'"
              >mdi-help-rhombus</v-icon
            >
            <v-progress-circular
              v-else-if="playbackStatus(user.index) == 'buffering'"
              indeterminate
            />
            <v-icon
              size="x-large"
              v-else-if="playbackStatus(user.index) == 'playing'"
              >mdi-play</v-icon
            >
            <v-icon
              size="x-large"
              v-else-if="playbackStatus(user.index) == 'paused'"
              >mdi-pause</v-icon
            >
          ...
          <div class="d-flex flex-column">
            <div>
              ...
              <v-tooltip
                text="Room leader"
                v-if="user.index == syncStore.session?.leader"
                location="top"
                offset="0"
              >
                <template v-slot:activator="{ props }">
            ...
            <div>
              <span v-if="playbackStatus(user.index) == 'nothing-playing'"
    ...
    <div class="flex-grow-1 ml-2 chat-box justify-end flex-shrink-1">
      <div
        v-for="(message, index) in syncStore.chat"
        :key="index"
        :class="message.member === 'system' ? 'chat-msg-system' : 'chat-msg'"
      >
        <span v-if="message.member !== 'system'" class="chat-msg-sender">{{
          message.member
        }}</span
        >{{ message.message }}
      </div>
  ...
  ******************************
  ...
  <div class="sidebar sidebar-undocked d-flex flex-column" v-else>
    ...
    <div v-for="user in syncStore.session?.members" :key="user.index">
      <v-slide-x-reverse-transition>
        <div
          v-if="playbackStatus(user.index) !== 'playing' || showingDock"
    ...
    <div
      :class="'flex-grow-1 chat-box justify-end flex-shrink-1 floating-chat fade-in-bg ' + (showingDock ? ' floating-bg' : '')"
    >
      <div
        v-for="(message, index) in syncStore.chat"
        :key="index"
