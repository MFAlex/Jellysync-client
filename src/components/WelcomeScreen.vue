      ...
      <v-dialog max-width="600" persistent no-click-animation>
        ...
        <template v-slot:default="{ isActive }">
          <v-card>
            ...
            <span class="ml-4 text-h6 mt-2">Language</span>
  ...
  methods: {
    ...
    async createRoomVue(server: string) {
      if (this.authStore.displayName != null) {
        ...
        const result = await createRoom(server, this.authStore.displayName);
    ...
    async joinRoomVue(code: string) {
      if (this.authStore.displayName != null) {
        ...
        const result = await joinRoom(code, this.authStore.displayName);
